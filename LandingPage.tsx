...
export default function LandingPage({ onGetStarted }: LandingPageProps) {
  return (
    <div className="min-h-screen flex flex-col">
      {/* ── Nav ─────────────────────────────────────────────────────── */}
      <nav className="bg-[#30003b] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <img src={LOGO_URL} alt="MyCareerBrain" className="w-10 h-10 rounded-xl object-contain" />
          <span className="font-norwester text-xl text-white tracking-tight">MyCareerBrain</span>
        </div>
        <button
          onClick={onGetStarted}
          className="px-6 py-2.5 bg-[#11ccf5] text-[#30003b] rounded-xl font-bold text-sm hover:bg-[#0ea5c9] transition-colors"
        >
          Sign In
        </button>
      </nav>
      ...
      {/* ── Hero ────────────────────────────────────────────────────── */}
      <section className="bg-[#30003b] text-white px-6 py-24 md:py-32 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          <img src={LOGO_URL} alt="" className="w-20 h-20 rounded-2xl object-contain mx-auto mb-8" />
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button
              onClick={onGetStarted}
              className="px-10 py-4 bg-[#11ccf5] text-[#30003b] rounded-2xl font-black text-lg hover:bg-[#0ea5c9] transition-all shadow-lg shadow-[#11ccf5]/20 flex items-center gap-2 active:scale-95"
            >
              Get Started <ChevronRight size={20} />
            </button>
            <button
              onClick={onGetStarted}
              className="px-10 py-4 border-2 border-white/20 text-white rounded-2xl font-bold text-lg hover:border-[#11ccf5] hover:text-[#11ccf5] transition-all"
            >
              Sign In
            </button>
          </div>
      ...
      <section className="bg-white px-6 py-20 md:py-28">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-3xl md:text-4xl text-center text-[#30003b] mb-4">
            How It Works
          </h2>
      ...
      {/* ── CTA Band ────────────────────────────────────────────────── */}
      <section className="bg-[#30003b] px-6 py-20 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <button
            onClick={onGetStarted}
            className="px-12 py-4 bg-[#11ccf5] text-[#30003b] rounded-2xl font-black text-lg hover:bg-[#0ea5c9] transition-all shadow-lg shadow-[#11ccf5]/20 active:scale-95"
          >
      ...
      {/* ── Footer ──────────────────────────────────────────────────── */}
      <footer className="bg-[#1a0020] px-6 py-4 flex items-center justify-center gap-4 text-[11px] text-white/30">
...
function BenefitCard({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) {
  return (
    <div className="bg-white border border-slate-100 rounded-[2rem] p-8 text-center hover:shadow-xl hover:shadow-[#11ccf5]/5 transition-all group">
